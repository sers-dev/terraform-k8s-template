resource "kubernetes_deployment_v1" "deployment" {
  count            = var.podResourceType == "deployment" ? 1 : 0
  wait_for_rollout = var.tfWaitForRollout
  ...
  spec {
    ...
    strategy {
      ...
      dynamic "rolling_update" {
        for_each = local.pvcEnabled ? [] : [1]
        content {
          max_surge       = var.podResourceTypeConfig.rollingUpdate.maxSurge
    ...
    min_ready_seconds         = var.podResourceTypeConfig.minReadySeconds
    progress_deadline_seconds = var.podResourceTypeConfig.deadlineSeconds
    revision_history_limit    = var.podResourceTypeConfig.revisionHistoryLimit
    ...
    template {
      ...
      spec {
        ...
        restart_policy      = var.podResourceTypeConfig.restartPolicy
        ...
        dynamic "volume" {
          for_each = local.pvcEnabled ? [1] : []
          content {
            name = kubernetes_persistent_volume_claim_v1.pvc.0.metadata.0.name
        ...
        dynamic "init_container" {
          ...
          content {
            ...
            dynamic "volume_mount" {
              for_each = local.pvcEnabled ? var.persistence.mounts : []
              content {
                name       = kubernetes_persistent_volume_claim_v1.pvc.0.metadata.0.name
                mount_path = volume_mount.value.containerPath
                sub_path   = volume_mount.value.volumePath
              }
            }
