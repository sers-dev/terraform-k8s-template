resource "kubernetes_cron_job_v1" "cronJob" {
  count = var.podResourceType == "cronjob" ? 1 : 0
  ...
  spec {
    concurrency_policy            = var.podResourceTypeConfig.concurrencyPolicy
    failed_jobs_history_limit     = var.podResourceTypeConfig.failedJobsHistoryLimit
